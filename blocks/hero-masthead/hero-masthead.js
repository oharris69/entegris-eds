...
export default function decorate(block) {
  ...
  // --- Image-only masthead detection ---
  // When the hero carries only an asset (image/video) with no meaningful text,
  // badge or CTA, it is a plain full-width banner masthead — not a text overlay.
  // Flag it so CSS can render an in-flow banner (real aspect ratio, no 700px
  // overlay box) instead of the centered overlay treatment.
  const hasAsset = !!block.querySelector('picture, img, video')
    || (assetDiv && !!assetDiv.querySelector('a'));
  const hasText = textDiv ? hasMeaningfulContent(textDiv) : false;
  const isImageOnly = hasAsset && !hasText && !badgeText && !ctaLabel;
  if (isImageOnly) block.classList.add('image-only');
  ...
  if (textDiv) textDiv.classList.add('hero-masthead-text');

  // --- Auto-detect video from any <a> in the block ---
  const videoLink = findVideoLink(block);

  if (videoLink) {
    block.classList.add('hero-masthead-video');
    ...
    video.className = 'hero-masthead-video-bg';
  ...
  if (badgeText && textDiv) {
    ...
    badgeEl.className = 'hero-masthead__badge';
