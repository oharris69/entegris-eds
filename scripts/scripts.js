...
function appendSource(picture, { type, srcset, media }) {
  ...
  src.srcset = srcset;
  if (media) src.setAttribute('media', media);
  picture.append(src);
}
...
function renderScene7Picture(src, alt) {
  ...
  image.src = buildScene7Rendition(src, { width: 750, format: 'jpg' });
  image.alt = alt;
  image.loading = 'lazy';
...
function renderDmOpenApiPicture(src, alt) {
  ...
  const image = document.createElement('img');
  image.src = buildDmOpenApiRendition(src, { width: 750 });
  image.alt = alt;
  image.loading = 'lazy';
  picture.append(image);
  return picture;
...
/* eslint-disable no-restricted-syntax, no-continue, no-await-in-loop, no-shadow, no-use-before-define, no-useless-escape, max-len, no-console */
...
/* eslint-enable no-restricted-syntax, no-continue, no-await-in-loop, no-shadow */
/* eslint-enable no-use-before-define, no-useless-escape, max-len, no-console */
